/**
 * @fileOverview block content script
 * @name block.js
...
window.addEventListener('load', async () => {
    ...
    // UI初期化
    UI.initialize(TwitSideModule.WINDOW_TYPE.BLOCK);
...
const showUsers = async () => {
    ...
    await TwitSideModule.ManageColumns.addColumn(
        TwitSideModule.TL_TYPE.TEMP_BLOCK,
        '', ownid,
        { onstart    : false,
          autoreload : false,
          notif      : false,
...
// ユーザ削除（ブロック）
const onClickDestroyBlockUser = (tweetBox) => {
    const boxid = tweetBox.id.replace(/^[a-zA-Z]{5}_/, ''); // columnidを除去

    UI.confirm(browser.i18n.getMessage('confirmUnblock'),
               () => {
